...
menuToggle.addEventListener('click', () => {
    ...
    const productData = {
        ...
        features: [
            "Akses model terbaru: GPT-4, GPT-4o, & GPT-5",
            "Mode reasoning o1 & o1-mini tanpa batas",
            "Bisa upload file & gambar untuk analisis",
            "Chat lebih cepat & prioritas server",
            "Kuota GPT-5 lebih besar untuk pengguna Plus",
        ...
        { name: 'Team+ - 1 Bulan', price: 15000, oldPrice: 30000, discount: 50 },
         { name: 'Plus - 1 Tahun', price: 20000, oldPrice: 100000, discount: 80 },
        ]
    };
    ...
    const openModal = (variant) => {
        orderModal.innerHTML = `
            ...
            <div class="info-row" style="color:#22c55e;">Proses Dilakukan Manual Lewat Wa</div>
    ...
    productData.variants.forEach((variant, index) => {
        ...
        item.innerHTML = `<div class="variant-top"><span class="variant-name">${variant.name}</span><div class="selector"></div></div><hr class="variant-divider"><div class="variant-bottom"><div class="variant-stock"><i class="fa-solid fa-wand-sparkles"></i> Ask Stock</div><div class="variant-pricing"><span class="current-price">Rp. ${variant.price.toLocaleString('id-ID')}</span><div class="old-price-container"><span class="old-price">Rp. ${variant.oldPrice.toLocaleString('id-ID')}</span><span class="discount">${variant.discount}%</span></div></div></div>`;
